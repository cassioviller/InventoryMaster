...
import { ArrowLeft, Plus, X, Package, Building, AlertTriangle } from 'lucide-react';
import { Link } from 'wouter';

// Schema para devolução de terceiro
...
const thirdPartyReturnSchema = z.object({
  date: z.string().min(1, 'Data é obrigatória'),
  thirdPartyId: z.number().min(1, 'Terceiro é obrigatório'),
  items: z.array(returnItemSchema).min(1, 'Adicione pelo menos um material'),
  notes: z.string().optional(),
});

type ThirdPartyReturnForm = z.infer<typeof thirdPartyReturnSchema>;
...
export default function ThirdPartyReturn() {
  ...
  // Form setup
  const form = useForm<ThirdPartyReturnForm>({
    resolver: zodResolver(thirdPartyReturnSchema),
    defaultValues: {
  ...
  const createReturnMutation = useMutation({
    mutationFn: async (data: ThirdPartyReturnForm) => {
      return apiRequest('/api/movements/third-party-return', {
        method: 'POST',
        body: JSON.stringify({
          type: 'entry',
          date: data.date,
          originType: 'third_party_return',
          returnThirdPartyId: data.thirdPartyId,
          isReturn: true,
          notes: data.notes,
  ...
  const onSubmit = (data: ThirdPartyReturnForm) => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold">Devolução de Terceiro</h1>
        </div>
      </div>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Terceiro que está devolvendo</FormLabel>
                      <Select onValueChange={(value) => field.onChange(parseInt(value))} value={field.value?.toString()}>
                        ...
                        <SelectContent>
                          {Array.isArray(thirdParties) && thirdParties.map((thirdParty: ThirdParty) => (
                            <SelectItem key={thirdParty.id} value={thirdParty.id.toString()}>
                              {thirdParty.name} - {thirdParty.document}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <Textarea
                  ...
                  placeholder="Ex: Final do contrato, material defeituoso, troca de especificação..."
                  rows={2}
                />
              </div>
